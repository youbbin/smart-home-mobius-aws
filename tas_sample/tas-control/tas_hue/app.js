...
function control_hue_led(controlData){
    //console.log("json_data : "+json_data)
    var option = null;
    var userid = "bGrTbuVoNJSmjqadzDfxUqZjB1Imd0LoGWCPAPXA";
    if(controlData.light_num == 0){ // 모든 조명 제어
        console.log("모든 조명 제어")
        option = {
            url : 'http://210.102.142.26:6180/api/'+userid+'/groups/0/action',
            body : controlData.control,
            json : true
        }
    ...
    else if(controlData.light_num == 1 || controlData.light_num == 2 || controlData.light_num == 3){
        option = {
            url : 'http://210.102.142.26:6180/api/'+userid+'/lights/'+controlData.light_num+'/state',
            body : controlData.control,
            json : true
        }
        request.put(option, function(err,response,body){
